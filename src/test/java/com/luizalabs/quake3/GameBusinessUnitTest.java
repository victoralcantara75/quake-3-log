...
class GameBusinessUnitTest {
    ...
    @Test
    @Transactional
    void shouldReturnGameSummary() throws IOException {
        ...
        Long id = game.get(0).getId();

        GameDTO gameDTO = gameBusiness.getGameSummaryById(id);

        Assert.assertEquals(1, (int)gameDTO.getTotal_kills());
        Assert.assertEquals(1, gameDTO.getPlayers().size());

    }

    @Test
    @Transactional
    void shouldReturnAllGamesSummary() throws IOException {
